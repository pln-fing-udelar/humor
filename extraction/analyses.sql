...
SELECT
  ...
  STDDEV(votes)
FROM
  (SELECT COUNT(gq.tweet_id) votes
...
SELECT
  COUNT(IF(vote = '1', 1, NULL)) / COUNT(*) v1,
...
SELECT
  ...
  COUNT(*)
FROM
  (SELECT IF(c = 0, 'No annotations', IF(p = x, 'Tie', IF(x > p, 'Not humorous',
...
SELECT
  ...
  COUNT(IF(vote = '4', 1, NULL)) v4,
  COUNT(IF(vote = '5', 1, NULL)) v5
FROM tweets t LEFT JOIN votes v ON (t.tweet_id = v.tweet_id)
GROUP BY t.tweet_id
